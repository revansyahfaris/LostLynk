...
const Lapor = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Foto wajib untuk laporan temuan
    if (tab === 'temuan' && !fotoFile) {
      alert('Foto barang wajib diunggah untuk laporan temuan!');
      return;
    }
    ...
    <div className="container" style={{ paddingTop: '2rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {/* Upload box — border merah & keterangan wajib jika tab temuan & belum ada foto */}
          <label style={{
            border: tab === 'temuan' && !fotoFile ? '2px dashed #ef4444' : '2px dashed var(--accent)',
            background: tab === 'temuan' && !fotoFile ? '#fff5f5' : 'var(--cream)',
            borderRadius: '16px', padding: '2rem', display: 'flex', flexDirection: 'column',
            alignItems: 'center', justifyContent: 'center', height: '200px',
            ...
            {previewImage ? (
              ...
              <>
                ...
                <p style={{ fontWeight: '500', color: tab === 'temuan' && !fotoFile ? '#ef4444' : 'var(--accent-mid)', margin: '0 0 0.25rem 0' }}>
                  Unggah Foto Barang
                  {tab === 'temuan' && <span style={{ color: '#ef4444', marginLeft: '3px' }}>*</span>}
                </p>
                <span style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Klik untuk upload</span>
                {tab === 'temuan' && !fotoFile && (
                  <span style={{ fontSize: '0.7rem', color: '#ef4444', marginTop: '0.4rem', fontWeight: '500' }}>
                    Wajib untuk laporan temuan
                  </span>
                )}
              </>
            )}
          ...
          <div style={{ background: '#ffffff', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.2rem' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: 'var(--muted)', display: 'block', marginBottom: '0.5rem' }}>PREVIEW LAPORAN</span>
